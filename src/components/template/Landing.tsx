    ...
    <>
      <section id="hero" className="grid grid-cols-1 min-h-[480px] items-center px-5 md:mx-8 mx-5 my-5 bg-muted rounded-md">
        <div className="space-y-3 col-span-1 text-center">
          <h1 className="text-4xl STablet:text-5xl md:text-6xl font-bold" style={{lineHeight: "1.2"}}>Empower Your <br /> Auto Gallery Seamlessly</h1>
          <p className="STablet:text-lg">Manage, showcase, rent, and sell cars in one place</p>
          <p className="max-w-3xl text-sm mx-auto STablet:text-base">or someone seeking to rent or purchase the perfect car, our platform provides seamless solutions for both showcasing and acquiring vehicles. Explore, rent, or buy with ease!</p>
        </div>
      </section>
      <section id="actions" className="px-5 md:px-8 pb-10">
        <div className="flex items-start desktop:flex-nowrap flex-wrap gap-4">
          <Link href={"/rent-car"} className={`${buttonVariants({ variant: "default", size: "lg" })} group h-48 rounded-lg flex-col flex-grow md:basis-2/6 py-8 !text-lg !items-start !justify-start`}>
            Rental Cars
          ...
          <Link href={"/buy-car"} className={`${buttonVariants({ variant: "default", size: "lg" })} group h-48 rounded-lg flex-col flex-grow md:basis-2/6 py-8 !text-lg !items-start !justify-start`}>
            Sale Cars
            <h5 className="text-sm text-muted-foreground">Wanna buy a car? check out sale cars.</h5>
            <FiArrowRight size={24} className="mt-auto text-muted-foreground self-end transition-transform duration-300 group-hover:translate-x-0 group-hover:text-white translate-x-[-0.5rem]"/>
          </Link>
          <Link href={"/galleries"} className={`${buttonVariants({ variant: "default", size: "lg" })} group h-48 rounded-lg flex-col flex-grow md:basis-2/6 py-8 !text-lg !items-start !justify-start`}>
            Galleries
