...
export const PATCH = async (req: Request, { params }: requestProps) => {
  try {
    ...
    const { title, buildYear, model, seats, fuelType, category, price, description="", mileage, color, is_published, deleted_images_id, updated_images_id_and_index } = formDataObject;
      ...
      if (updated_images_id_and_index_array !== undefined && updated_images_id_and_index_array.length > 0) {
        updated_images_id_and_index_array.forEach(async (updatedImage: { id: string, index: number }) => {
          ...
          if (index < parsedImagesFile.length + 1) {
            ...
            const { data, error } = await supabase.storage.from("cars").upload(`car_${existingAutoGallery.id}` + "/sale" + `/${Date.now()}_image`, imageToUpdate, { cacheControl: '3600', upsert: true });
